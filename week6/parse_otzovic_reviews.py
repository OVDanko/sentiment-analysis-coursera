...
def parse_page(sm_link, proxy_list):
    ...
    if (type(req) is int): # если ни один запрос не удался
        ...
        x = parser.find('div', attrs={'class': "review-body description"}).text
        yes = parser.find('td', attrs={'class': "recommend-ratio yes"})
        ans = 1 if yes else 0
        print('done: ', sm_link)
        with codecs.open('responses_texts.txt', 'a', 'utf-8') as output_file:
            output_file.write(str(ans) + ',' + str(x) + '\n')
        return (ans, x)
